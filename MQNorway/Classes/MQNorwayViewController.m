...
- (UIView *)tiledScrollView:(TiledScrollView *)tiledScrollView tileForRow:(int)row column:(int)column resolution:(int)resolution {
	...
	[tile setImage:[UIImage imageNamed:[NSString stringWithFormat:@"Norge_%d_%d_%d.jpg", resolutionPercentage, column, row]]];
...
- (void)setZoomScale:(CGSize)size {
    ...
    float minScale = [playingBoardView frame].size.width  / size.width;
