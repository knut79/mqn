...
-(void) SetTilesMapViewBounds
{
	...
	CGRect testRect = CGRectMake(0, 0, 320, 480);
	testRect.origin.y += (centerPoint.y - (480/2));
	if (testRect.origin.y > (800 - 480)) 
	{
		testRect.origin.y = 800 - 480;
	}
...
//				mapSize.height = mapSize.height * (tiledMapViewResolutionPercentage/100);
				...
				for (int row = minRow; row <= maxRow; row++) {
					for (int col = minCol; col <= maxCol; col++) {
						...
						imageName = [NSString stringWithFormat:@"Norge_%d_%d_%d.jpg",(int)tiledMapViewResolutionPercentage, col, row];
						...
						CGContextDrawImage(context, CGRectMake(scaledTileWidth * col,-scaledTileHeight * row + (-scaledTileHeight) + endOfColValue,scaledTileWidthDynamic,scaledTileHeightDynamic), image);
